...
export default function WeeklyReportTab({ workerId }: WeeklyReportTabProps) {
  ...
  const fetchWeeklyReport = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await workersAPI.getWeeklyReport(workerId);
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-y-auto max-h-[calc(100vh-22rem)] shadow-lg">
        ...
        <tbody>
            ...
            return (
              ...
              >
                ...
                <td className={`${tdBase} text-text-primary border-l border-border`}>
                  {week.attendanceCount > 0 ? week.attendanceCount : '-'}
                </td>
                <td className={`${tdBase} text-text-primary`}>
                  {week.otUnits > 0 ? week.otUnits : '-'}
                </td>
          ...
          <tr className="bg-surface border-t-2 border-border sticky bottom-0 z-10">
            ...
            <td className={`${tdBase} font-bold text-blue-300 border-l border-border`}>
              {totals.attendanceCount > 0 ? totals.attendanceCount : '-'}
            </td>
            <td className={`${tdBase} font-bold text-blue-300`}>
              {totals.otUnits > 0 ? totals.otUnits : '-'}
            </td>
            <td className={`${tdBase} font-bold text-blue-300`}>
